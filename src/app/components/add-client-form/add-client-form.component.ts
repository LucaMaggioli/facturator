...
export class AddClientFormComponent implements OnInit {
  ...
  emailInvalidErrorMessage() {
    return this.email.hasError('email') ?
      'Not a valid email' : this.email.hasError('required')?
        'You must enter a value': '';
  }

  isValidForm():boolean{
    return this.lName.valid && this.fName.valid && this.address.valid && this.email.valid;
  }

  sendNewClientEvent(){
    let fullName = this.fName.value + ' - ' + this.lName.value;
    let newClient = new Client(fullName, this.address.value, this.email.value);
