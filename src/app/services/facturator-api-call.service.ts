...
export class FacturatorApiCallService {
  ...
  async getCLients(){
    return await fetch(this.backendURL + "/client/all", {method: 'GET'}).then(response => response.json());
  }
  ...
  async getArticles(){
    return await fetch(this.backendURL + "/article/", {method: 'GET'}).then(response => response.json());
  }
}
